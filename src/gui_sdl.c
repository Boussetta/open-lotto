...
int gui_render_frequency_2d(const char *title, const FrequencyReport *report, int dark_mode)
{
    ...
    SDL_Window *win =
        SDL_CreateWindow(title ? title : "Frequency Distribution", SDL_WINDOWPOS_CENTERED,
                         SDL_WINDOWPOS_CENTERED, WINDOW_WIDTH, WINDOW_HEIGHT, SDL_WINDOW_SHOWN);
    if (!win)
    {
        TTF_Quit();
    ...
    while (running)
    {
        ...
        SDL_Color bar_c =
            dark_mode == 1 ? (SDL_Color){255, 215, 0, 230} : (SDL_Color){40, 120, 230, 255};
        ...
        /* Title */
        char header[160];
        snprintf(header, sizeof(header), "Frequency Distribution  |  game: %s  |  draws: %d",
        ...
        {
            char date_str[128];
            snprintf(date_str, sizeof(date_str), "Period: %s to %s", report->from_date,
                     report->to_date);
        ...
        if (tooltip_number >= 0 && font_sm && anim >= 1.0f)
        {
            ...
            double pct =
                report->total_draws > 0 ? (100.0 * count) / (double)report->total_draws : 0.0;
...
int gui_render_barometer_2d(const char *title, const BarometerReport *report, int dark_mode)
{
    ...
    SDL_Window *win =
        SDL_CreateWindow(title ? title : "Barometer — Overdue Factor", SDL_WINDOWPOS_CENTERED,
                         SDL_WINDOWPOS_CENTERED, WINDOW_WIDTH, WINDOW_HEIGHT, SDL_WINDOW_SHOWN);
    if (!win)
    {
        TTF_Quit();
    ...
    while (running)
    {
        ...
        SDL_Color dim =
            dark_mode == 1 ? (SDL_Color){40, 50, 65, 255} : (SDL_Color){200, 210, 225, 255};
        SDL_Color bar_c =
            dark_mode == 1 ? (SDL_Color){255, 145, 50, 230} : (SDL_Color){210, 80, 30, 255};
        ...
        char header[160];
        snprintf(header, sizeof(header), "Barometer — Overdue Factor  |  game: %s  |  draws: %d",
                 title ? title : "", report->total_draws);
        draw_text(ren, font, header, left, 18, fg);
...
int gui_render_hot_cold_2d(const char *title, const HotColdReport *report, int dark_mode)
{
    ...
    SDL_Window *win =
        SDL_CreateWindow(title ? title : "Hot / Cold Numbers", SDL_WINDOWPOS_CENTERED,
                         SDL_WINDOWPOS_CENTERED, WINDOW_WIDTH, WINDOW_HEIGHT, SDL_WINDOW_SHOWN);
    if (!win)
    {
        TTF_Quit();
    ...
    while (running)
    {
        ...
        char header[160];
        snprintf(header, sizeof(header), "Hot / Cold  |  game: %s  |  draws: %d  |  top %d",
        ...
        for (int i = 0; i < groups; i++)
        {
            ...
            int h_hot =
                (int)flerpf(0.0f, (float)(report->hot[i].count * chart_h / max_count), anim);
            int h_cold =
                (int)flerpf(0.0f, (float)(report->cold[i].count * chart_h / max_count), anim);
        ...
        {
            const HotColdEntry *e = tip_is_hot ? &report->hot[tip_group] : &report->cold[tip_group];
            char line1[40], line2[48];
            snprintf(line1, sizeof(line1), "Ball %d  (%s rank %d)", e->number,
                     tip_is_hot ? "HOT" : "COLD", tip_group + 1);
            snprintf(line2, sizeof(line2), "%d draws  (%.1f%%)", e->count, e->percentage);
            draw_tooltip(ren, font_sm, line1, line2, tip_x, tip_y, WINDOW_WIDTH, WINDOW_HEIGHT);
        }

        /* Legend box */
        int lx = right - 90, ly = top + 6;
        SDL_Rect box = {lx - 8, ly - 4, 88, 46};
        SDL_SetRenderDrawColor(ren, dark_mode == 1 ? 30 : 210, dark_mode == 1 ? 34 : 215,
                               dark_mode == 1 ? 46 : 228, 200);
